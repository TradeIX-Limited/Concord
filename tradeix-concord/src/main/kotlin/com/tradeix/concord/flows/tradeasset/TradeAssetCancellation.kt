...
import com.tradeix.concord.validators.tradeasset.TradeAssetCancellationFlowModelValidator
...
object TradeAssetCancellation {
    ...
    class InitiatorFlow(private val model: TradeAssetCancellationFlowModel) : FlowLogic<SignedTransaction>() {
        ...
        private fun verify(initiator: Party, state: TradeAssetState) {
            ...
            if (initiator == state.buyer && state.status == TradeAssetState.TradeAssetStatus.INVOICE) {
                errors.add(EX_BUYER_CANNOT_CANCEL_INVOICE)
            }

            if (initiator == state.supplier && state.status == TradeAssetState.TradeAssetStatus.PURCHASE_ORDER) {
