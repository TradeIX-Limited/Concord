...
class FundingResponseAcceptanceFlowTests : FlowTest() {
    ...
    @Test
    fun `Funding response acceptance flow must be signed by the initiator`() {
        val transaction = FundingResponseFlows.accept(
                network = network,
                initiator = supplier1.node,
                message = FUNDING_RESPONSE_ACCEPTANCE_REQUEST_MESSAGE
        )
    ...
    @Test
    fun `Funding response acceptance flow must be signed by the acceptor`() {
        val transaction = FundingResponseFlows.accept(
                network = network,
                initiator = supplier1.node,
                message = FUNDING_RESPONSE_ACCEPTANCE_REQUEST_MESSAGE
    ...
    @Test
    fun `Funding response acceptance flow records a transaction in all counter-party vaults`() {
        val transaction = FundingResponseFlows.accept(
                network = network,
                initiator = supplier1.node,
                message = FUNDING_RESPONSE_ACCEPTANCE_REQUEST_MESSAGE
    ...
    @Test
    fun `Funding response acceptance flow has only one input and only one output`() {
        val transaction = FundingResponseFlows.accept(
                network = network,
                initiator = supplier1.node,
                message = FUNDING_RESPONSE_ACCEPTANCE_REQUEST_MESSAGE
